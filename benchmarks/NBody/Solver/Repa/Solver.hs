...
calcAccelsWithBoxR epsilon llx lly rux ruy mpts
 = let  mpts'   = R.map (\(x,y,m) -> MP x y m) mpts
   in   calcAccelsWithBox epsilon llx lly rux ruy mpts'
...
splitPoints
        :: BoundingBox
        -> Array U DIM1 MassPoint
        -> (Array U DIM1 BoundingBox, Array U DIM2 MassPoint)

splitPoints b@(Box llx lly rux  ruy) particles 
  | noOfPoints <= 1 = (singletonP b, singletonP particles)
  | otherwise         
  = unzipP [: (b,p) | (b,p) <- zipP boxes splitPars, lengthP p I.> 0:]
  where noOfPoints      = size (extent particles)
